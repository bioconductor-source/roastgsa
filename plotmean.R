...
plotMean <- function(obj, whplot = 1, maintitle = "", statistic = "mean",
                     ylimAll = TRUE, ylim = NULL,  maxDensPlot = 20,
  ...
  for(K in names(index2)){
      ...
      if(gsainfo) mtext(subtitle, 3, line=0.5, col= "darkgray", cex = cex.sub)
...
#      myTicks <- axTicks(2);
      ...
      plot(NA,NA, ylim=ylim, xlim=c(0,1),axes=FALSE,xlab='',ylab='',sub="",col='green',lwd=lwd)
      abline(h=es[,K],col=myCols)
      maxmin <- ylim[2] - ylim[1]
      polygon( c(0-0.05,0-0.05,1+0.05,1+0.05), c(ylim[1], ylim[1] + maxmin/15, ylim[1] + maxmin/15, ylim[1]),
              col= rgb(0, 0, 0.5,0.3), border = NA)
      polygon(c(0-0.05,0-0.05,1+0.05,1+0.05), c(ylim[2], ylim[2] - maxmin/15, ylim[2] - maxmin/15, ylim[2]),
